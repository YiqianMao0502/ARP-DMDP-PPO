...
class Env2DCylinder(Environment):
    ...
    def __init__(self, path_root, geometry_params, flow_params, solver_params, output_params,
        ...
        self.list_delayed_actions = np.zeros(self.delayed_times)
    ...
    def reset(self):
        ...
        this_state = np.transpose(np.array(self.probes_values))
        #next_state = np.transpose(np.concatenate((self.list_delayed_actions, np.array(self.probes_values)),axis=1))
        trans_delayed_actions= np.transpose(self.list_delayed_actions)
        next_state = np.concatenate((trans_delayed_actions, this_state),axis=0)
...
#             pickle.dump(self.list_save_reward, f, pickle.HIGHEST_PROTOCOL)
    ...
    def execute(self, actions=None):
        try:
            ...
            self.list_delayed_actions=np.delete(self.list_delayed_actions, [0,1,2,3])
            self.list_delayed_actions=np.append(self.list_delayed_actions, action)
            this_state = np.transpose(np.array(self.probes_values))
            #next_state = np.transpose(np.concatenate((self.list_delayed_actions, np.array(self.probes_values)),axis=1))
            trans_delayed_actions= np.transpose(self.list_delayed_actions)
            next_state = np.concatenate((trans_delayed_actions, this_state),axis=0)
    ...
    def compute_reward(self):
        ...
        if self.reward_function == 'plain_drag':  # a bit dangerous, may be injecting some momentum
            ...
            total_reward = avg_drag + 0.2 - 0.2 * abs(avg_lift)
            name = "reward.csv"
            if (not os.path.exists("total_reward")):
                os.mkdir("total_reward")
            if (not os.path.exists("total_reward/" + name)):
                with open("total_reward/" + name, "w") as csv_file:
                    spam_writer = csv.writer(csv_file, delimiter=";", lineterminator="\n")
                    spam_writer.writerow(["Name", "Episode", "Step", "avg_length", "avg_drag", "avg_lift", "total_reward"])
                    spam_writer.writerow([self.simu_name,
                                          self.episode_number,
                                          self.solver_step,
                                          avg_length,
                ...
                with open("total_reward/" + name, "a") as csv_file:
                    spam_writer = csv.writer(csv_file, delimiter=";", lineterminator="\n")
                    spam_writer.writerow([self.simu_name,
                                          self.episode_number,
                                          self.solver_step,
    ...
    def states(self):
        if self.output_params["probe_type"] == 'pressure':
            ...
            return dict(type='float',
                        shape=(2 * len(self.output_params["locations"]) * self.optimization_params["num_steps_in_pressure_history"]+self.delayed_times, )
                        )
