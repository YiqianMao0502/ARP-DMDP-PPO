...
class Env2DCylinder(Environment):
    ...
    def reset(self):
        ...
        if self.state_initialization:
            next_state = np.concatenate((self.this_state, self.action, self.this_state),axis=0)
            self.state_initialization = False
        else:
            next_state = np.concatenate((self.timedelayed_state, self.action, self.this_state),axis=0)
...
#             pickle.dump(self.list_save_reward, f, pickle.HIGHEST_PROTOCOL)
    ...
    def states(self):
        if self.output_params["probe_type"] == 'pressure':
            ...
            return dict(type='float',
                        shape=(4 * len(self.output_params["locations"]) * self.optimization_params["num_steps_in_pressure_history"] + len(self.geometry_params["jet_positions"]), )
                        )
